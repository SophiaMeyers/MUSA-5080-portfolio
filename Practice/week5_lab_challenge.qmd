...
>
> Is that a β̂ problem or a ŷ problem? Write your answer here before you start:β_
...
No but it is significantly different than the null hypothesis because the p-value is very small so it is signifigant

2. **ŷ reading:** Should `n_stations` stay in a model whose only job is predicting rent? Why or why not?

Yes because it is signifigant so it is helping the model
...
**Stop and look:** Which RMSE is larger? Why would you expect that? In words: "The model's typical miss on tracts it hasn't seen is about $___ per month."
...
ctrl <- trainControl(method = "cv", number = ___)
